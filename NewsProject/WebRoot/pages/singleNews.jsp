	...
	<div id="page-content" class="single-page container">
		<div class="row">
			<div id="main-content" class="col-md-8">
				<div class="box">
					<div class="line"></div>
					<h1 class="vid-name"><a href="#">${news.title }</a></h1>
					<div class="info">
						<h5>作者： ${username}</h5>
						<span><i class="fa fa-calendar"></i><fmt:formatDate value="${news.createTime }" pattern="yyyy-MM-dd"/></span> 
						<span><i class="fa fa-heart"></i>13</span>
						<span><img src="${pageContext.request.contextPath }/${news.img}"></span>
					</div>
					...
					<div class="vid-tags">
						<a href="#">
						<c:choose>
					  <c:when test="${news.type==0 }">财经</c:when>
                      <c:when test="${news.type==1 }">社会</c:when>
                      <c:when test="${news.type==2 }">军事</c:when>
                      <c:when test="${news.type==3 }">历史文化</c:when>
                      <c:when test="${news.type==4 }">科技</c:when>
                      <c:when test="${news.type==5 }">汽车</c:when>
                      <c:when test="${news.type==6 }">房产</c:when>
                      <c:when test="${news.type==7 }">体育</c:when>
                      <c:when test="${news.type==8 }">娱乐</c:when>
                      <c:when test="${news.type==9 }">健康</c:when>
						</c:choose>
